...
const BlogTitles = () => {
  const blogCategories = ['General','Technology','Buisness','Health','LifeStyle','Education',
    'Travel','Food']
  ...
  const {getToken} = useAuth();
    const onSubmitHandler = async(e)=>{
      ...
      try {
        setLoading(true);
        const prompt = `Generate a title for the keyword ${input} in the category ${selectedCategory}`;
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {blogCategories.map((item)=>(
            ...
            className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedCategory===item?
              'bg-purple-50 text-purple-700' : 'text-gray-500 border-gray-300'}`}key={item}>{item}</span>
          ))}
        </div>
        <br />
