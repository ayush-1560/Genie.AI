    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <p className='mt-6 text-sm font-medium'>Describe your Image</p>
          ...
          border-gray-300' placeholder='Describe what you want to see in the image...' required/>
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {ImageStyle.map((item)=>(
            ...
            className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedStyle===item?
              'bg-green-50 text-green-700' : 'text-gray-500 border-gray-300'}`}key={item}>{item}</span>
          ))}
        </div>
      ...
      border-gray-200 min-h-96'>
        ...
        </div>
        {
          ...
          text-gray-400'>
            ...
            <div className='mt-3 h-full'><img src={content} alt="image" className='w-full h-full' /></div>
          )
        }
      </div>
    </div>
